...
public class DepartmentValidation {
	...
	public void validateAddDepartment(DepartmentDTO departmentDTO) {

		Optional<DepartmentDTO> optionalDepartmentDTO = DepartmentService.DEPARTMENTS.stream()
				.filter(e -> e.getDepartmentName().equalsIgnoreCase(departmentDTO.getDepartmentName())).findFirst();
	...
	public DepartmentDTO validateUpdateDepartment(DepartmentDTO departmentDTO, String departmentId) {
		...
		if (!presentDepartmentDTO.isPresent()) {
			...
			Optional<DepartmentDTO> optionalDepartmentDTO = DepartmentService.DEPARTMENTS.stream()
					.filter(e -> e.getDepartmentName().equalsIgnoreCase(departmentDTO.getDepartmentName()) && !e.getDepartmentId().equals(departmentId)).findFirst();
			if (optionalDepartmentDTO.isPresent()) {
				throw new ResourceFoundException("Name", departmentDTO.getDepartmentName());
			}
		}
